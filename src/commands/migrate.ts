...
export function migrate(options: MigrateOptions): number {
	...
	say('    1. Dockerfile          rebuild it on the published image: `devc init --yes --no-install`')
	say('                           into an empty directory writes the fw-bake template; keep your own')
	say('                           RUN lines between its final FROM and USER node, and drop a v2')
	say(`                           "COPY firewall/" + firewall-docker-setup.sh pair — the bake stage does`)
	say(`                           that now. The ${LOCAL_LINEAGE} lineage ends here.`)
	...
	say('                           and skills/sync-skills.sh — the 1.2.0 image lines still prefer a')
	say('                           workspace loader (1.3.0 does not), which would install your skills')
	say('                           alone and drop the baked ones. To this CLI the tree is v3 from here on.')
