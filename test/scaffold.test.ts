...
test('the plan is exactly the D15 tree', () => {
	...
	assert.equal(plan.imageRef, 'ghcr.io/meitogi/devcontainer-sandbox:1.5.0-cc2.1.280')
})
...
test('.env carries the answers live on their documented lines', () => {
	...
	assert.match(env, /^BASE_IMAGE=ghcr\.io\/meitogi\/devcontainer-sandbox:1\.5\.0-cc2\.1\.220$/m)
